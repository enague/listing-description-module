...
let listing_names = [];
headers_names = "type,id,name,blurb,neighborhood\n";
fs.writeFileSync("faker_listing_names.csv", headers_names, "utf8");

for (let i = 1; i < 10000001; i += 1) {
	const type = "listing_names";
	const id = i;
	const name = faker.lorem.word();
	const blurb = faker.lorem.word();
	const neighborhood = faker.address.city();
	listing_names.push([type, i, name, blurb, neighborhood]);
...
let tblAmenities = [];
headers_amenities = "type,id,category,subcategory,additionalcomments\n";
fs.writeFileSync("faker_tblAmenities.csv", headers_amenities, "utf8");

for (let i = 1; i < 10000001; i += 1) {
	const type = "amenities";
	const id = i;
	const name = faker.lorem.word();
	const blurb = faker.lorem.word();
	const neighborhood = faker.address.city();
	tblAmenities.push([type, i, name, blurb, neighborhood]);
...
let tblOverview = [];
const headers =
	"type,id,summary,thespace,guestaccess,interactionswithguests,otherthingstonote,homehighlights1,homehighlights2,homehighlights3,houserules,noofguests,noofbeds,noofbedrooms,noofbeds\n";
...
headers_arrangements = "type,id,listingid,roomname,noofbeds,typeofbed\n";
fs.writeFileSync(
...
console.time("space");
let theSpace = [];
const headers_space = "type,id,thespace\n";
...
for (let i = 1; i < 10000001; i += 1) {
	...
	theSpace.push([type, id, thespace]);
